      ...
      <main className="container">
        <section className="card overview">
          <div className="overview-head">
            <h2>整体架构</h2>
            <p>前端构建产物（带 hash）{'->'} Nginx 静态服务 {'->'} 按类型分发不同 Cache-Control。</p>
          </div>
        ...
        <section className="card">
          ...
          <div className="pitfall-grid">
            <div className="pitfall-item">
              ...
              <p className="muted">
                Nginx 中，若子 location 使用了 <code className="inline-code">add_header</code>，会<strong>覆盖</strong>而非合并父级的 add_header。
                因此每个 location 都需显式声明完整头部，或用 <code className="inline-code">include</code> 公共片段。本配置已为每个 location 补全。
              </p>
            </div>
            <div className="pitfall-item">
              ...
              <p className="muted">
                若 HTML 也设长缓存，用户在缓存期内拿不到引用新 hash JS 的 HTML，发版后页面会加载旧的、可能已不存在的 JS 文件而白屏。
              </p>
            </div>
            <div className="pitfall-item">
              ...
              <p className="muted">
                只有文件名带 contenthash 才能用 immutable。若文件名固定（如 <code className="inline-code">app.js</code>），
                immutable 会导致用户永远拿不到更新。
              </p>
            </div>
            <div className="pitfall-item">
              ...
              <p className="muted">
                SW 文件本身被强缓存后，浏览器不会重新拉取，导致 SW 无法升级。必须 <code className="inline-code">no-store</code>。
              </p>
            </div>
            <div className="pitfall-item">
              ...
              <p className="muted">
                开启 gzip 后必须加 <code className="inline-code">Vary: Accept-Encoding</code>（gzip_vary on 自动加），
                否则 CDN 可能压缩版与未压缩版缓存串。
              </p>
            </div>
            <div className="pitfall-item">
              ...
              <h3>分布式 ETag 一致性</h3>
              <p className="muted">
                多台 Nginx 各自生成 ETag（基于 mtime/size）可能不同，命中下降。必要时关闭 ETag 仅用 Last-Modified，或前置 CDN 统一缓存。
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="card">
          <div className="card-head">
            ...
            <p>本方案的真实配置文件位于项目根目录 <code className="inline-code">nginx.conf</code>，可直接用于部署。</p>
          </div>
          <pre className="code-block file-path">03-nginx-full-config/nginx.conf</pre>
          <div className="note-box">
            <strong>提示：</strong>若不使用 Docker，可把 <code className="inline-code">nginx.conf</code> 内容放到
            <code className="inline-code">/etc/nginx/conf.d/default.conf</code>，并保证 <code className="inline-code">root</code>
