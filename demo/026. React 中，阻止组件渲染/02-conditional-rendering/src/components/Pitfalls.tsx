  ...
  const items: Array<{ title: string; bad: ReactNode; good: ReactNode }> = [
    {
      ...
      bad: (
        <span>
          <code>{'{count && <Comp/>}'}</code> 当 count=0 时会渲染出 "0"
        </span>
      ),
      good: (
        <span>
          <code>{'{count > 0 && <Comp/>}'}</code> 或 <code>{'{Boolean(count) && <Comp/>}'}</code>
        </span>
      ),
    },
    {
      ...
      bad: (
        <span>
          <code>{'{a ? (b ? X : Y) : (c ? Z : W)}'}</code> 可读性极差
        </span>
      ),
      good: (
        <span>
          抽成 <code>renderContent()</code> 函数或子组件，用 switch
        </span>
      ),
    },
    {
      ...
      bad: (
        <span>
          <code>{'{cond && useMemo(...)}'}</code> 把 hooks 放在条件分支里
        </span>
      ),
    ...
    },
    {
      ...
      bad: (
        <span>
          <code>{'{cond ? <A/> : <B/>'}</code> 切换时 React 可能复用错误的状态
        </span>
      ),
      good: (
        <span>
          给切换的元素加 <code>key</code> 强制重建，或用 <code>display:none</code> 隐藏
        </span>
      ),
    },
    {
      ...
      bad: (
        <span>
          频繁切换的 Tab 用 <code>{'{cond && <Tab/>}'}</code> 反复挂载卸载，性能差
        </span>
      ),
      good: (
        <span>
          频繁切换场景用 <code>display:none</code> 保活；不频繁的用条件渲染
        </span>
      ),
    },
    {
      ...
      bad: (
        <span>
          <code>{'{cond ? <Comp/> : null}'}</code> 多此一举
        </span>
      ),
      good: (
        <span>
          直接用 <code>{'{cond && <Comp/>}'}</code> 更简洁
        </span>
      ),
    },
  ]
