...
export const PitfallZero: FC = () => {
  ...
  return (
    <div className="page">
      ...
      <p>
        JavaScript 中 <code>0</code> 是 falsy，所以 <code>0 && {'<Comp/>'}</code> 的求值结果是 <code>0</code>。
        但 React 会把 <code>0</code> 当作合法的子节点，<b>在页面上渲染出字符 "0"</b>！
        这是 React 中最常见、最容易踩的条件渲染陷阱。
      </p>
      ...
      <div className="demo-area">
        <div className="row">
          ...
          <button className="btn-ghost" onClick={() => setCount((c) => c + 1)}>+1</button>
          <button className="btn-ghost" onClick={() => setCount(0)}>重置为 0</button>
        </div>
      ...
      <div className="note">
        <b>根因：</b>React 渲染子节点时，<code>false</code>、<code>null</code>、<code>undefined</code>、<code>true</code>
        会被忽略，但 <code>0</code> 和 <code>NaN</code> 会被当作有效文本节点渲染。
        所以 <code>{'{someNumber && <Comp/>}'}</code> 在 someNumber 为 0 时会输出 "0"。
        <b>修复：</b>用 <code>count &gt; 0 &&</code> 或 <code>Boolean(count) &&</code> 或 <code>!!count &&</code>。
      </div>
    </div>
  )
}
