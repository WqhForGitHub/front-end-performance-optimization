...
export default function Profile() {
  ...
  return (
    <div className="page">
      <h2>Profile 个人中心 <span className="tag tag-load">lazy chunk</span></h2>
      <p>
      ...
      <div className="diagram">
        ...
        <div style={{ fontFamily: 'Fira Code, Consolas, monospace', fontSize: 12, lineHeight: 1.8, color: '#334155' }}>
          <div>t=0ms    请求 index.html</div>
          <div>t=20ms   解析 main.js（含 App + 路由表，但不含页面实现）</div>
          <div>t=35ms   请求 vendor.js（react / react-dom）</div>
          <div>t=60ms   请求 Home.js（首屏路由）=&gt; <span style={{ color: '#10b981' }}>可渲染首屏</span></div>
      ...
      {editing ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 360 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="nav-btn active" onClick={() => setEditing(false)}>保存</button>
            <button className="nav-btn" onClick={() => setEditing(false)}>取消</button>
          </div>
        </div>
      ) : (
        <button className="nav-btn" onClick={() => setEditing(true)}>编辑资料</button>
      )}
