...
export default function Settings() {
  ...
  return (
    <div className="page">
      <h2>
        Settings 设置 <span className="tag tag-load">lazy chunk</span>
      </h2>
      <p>
      ...
      <div className="diagram">
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: '120px 1fr 1fr', gap: 8, fontSize: 13 }}
        >
          <div></div>
          ...
          <div style={{ color: '#475569' }}>
            必须拆分 + 预加载
            <br />
            <small>如带图表的 Dashboard</small>
          </div>
          <div style={{ color: '#475569' }}>
            必须拆分、按需加载
            <br />
            <small>如 Settings（当前页）</small>
          </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 440 }}>
        ...
        <Row label="语言">
          <select
            value={lang}
            onChange={(e: ChangeEvent<HTMLSelectElement>) => setLang(e.target.value as 'zh' | 'en')}
          >
        ...
        <Row label="消息通知">
          <input
            type="checkbox"
            checked={notify}
            onChange={(e: ChangeEvent<HTMLInputElement>) => setNotify(e.target.checked)}
          />
        </Row>
        <Row label="自动保存">
          <input
